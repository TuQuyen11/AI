...
#define empty 0
const char* action[]= {"First state", "pour Water Full X", "pour Water Full Y", "pour Empty X", "pour Empty Y","pour Water X to Y", "pour Water Y to X"};
typedef struct{
   int x;
   int y;
}State;
...
int call_operators(State cur_state, State *result, int opt){
   switch(opt){
      ...
      case 2:return pourWaterFullY(cur_state, result);
         break;
		case 3:return pourEmptyX(cur_state, result);
         break;
      case 4:return pourEmptyY(cur_state, result);
         break;
